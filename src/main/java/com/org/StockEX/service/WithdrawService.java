...
public ResponseEntity<?> withdraw(UpdateAccountBalanceDTO updateAccountBalanceDTO){
    int updatedRows = 0;
    ...
    updatedRows = userAccountRepo.withdrawAmount(amount, userId);
    ...
    userAccountHistoryRepo.save(history);
            ...
            return updatedRows > 0 ? ResponseEntity.ok(Map.of("message","₹"+amount+" Debited from your account.Your Current Balance is "+userAccountRepo.getAccountBalance(userId)+".")) : ResponseEntity.badRequest().body(Map.of("message","Something Went Wrong."));
}
